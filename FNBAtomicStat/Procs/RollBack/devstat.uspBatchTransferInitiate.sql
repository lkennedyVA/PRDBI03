...
/*
	...
	Example:
		...
		;
		EXEC [stat].[uspBatchTransferInitiate] @piStatBatchLogId = @iStatBatchLogId, @piBatchId = @iBatchId
		;
		SELECT StatBatchLogId = @iStatBatchLogId, BatchId = @iBatchId
		;
		SELECT * FROM [risk].[StatBatch] WHERE BatchId = @iBatchId
		;
...
*/
ALTER PROCEDURE [devstat].[uspBatchTransferInitiate]
	( 
...
BEGIN
	...
	UPDATE [risk].[StatBatch] 
		...
		WHERE StatBatchId = @iBatchId 
			AND [StatBatchCompletedDatetime] IS NOT NULL -- can't initiate the batch for transfer if it is still being prepared by the ???Risk process.
			AND [StatBatchTransferInitiatedDatetime] IS NULL -- can't initiate the batch for transfer if it is already marked as initiated.
	;
