...
GO
/****************************************************************************************
	Name: uspMTBDailyCustNegBalDays30and90Delta_TransferToAtomicStat
	Generator Executed By: VALIDRS\LWhiting
	Generated Version: B93DBC79-C497-4092-8583-92BDE168E2FD
	Description: Generated user-defined function.
		
	History:
		2019-04-23 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [mtb].[uspMTBDailyCustNegBalDays30and90Delta_TransferToAtomicStat]
	(
...
BEGIN
	...
	;
	SELECT @iMTBBatchDataSetRefreshLogId = mtb.ufnMTBBatchDataSetRefreshLogIdGetByBatchLogIdMTBDataSetName( @siStatBatchLogId, N'MTBDailyCustNegBalDays30and90Delta' )
	;
	
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iMTBBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iMTBBatchDataSetRefreshLogId ), N'NULL' ) + N' via mtb.ufnMTBBatchDataSetRefreshLogIdGetByBatchLogIdMTBDataSetName( ' + CONVERT( nvarchar(50), @siStatBatchLogId ) + N'}, N''MTBDailyCustNegBalDays30and90Delta'' )'
	END

	EXEC mtb.uspMTBBatchDataSetRefreshLogTransferInitiate
		 @piStatBatchLogId = @siStatBatchLogId
		,@pnvMTBDataSetName = N'MTBDailyCustNegBalDays30and90Delta'
	...
	IF @iMTBBatchDataSetRefreshLogId IS NOT NULL 
		AND EXISTS( SELECT 'X' FROM DHayes.dbo.MTBDailyCustNegBalDays30and90Delta x WHERE x.MTBBatchId = mtb.ufnMTBBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		WHERE [ObjectName] = N'MTBDailyCustNegBalDays30and90Delta'
		;
		SELECT @biDataSetRowCount = NumberOfRecordsInDataSetRefresh 
		FROM DHayes.dbo.MTBBatchDataSetRefreshLog 
		WHERE MTBBatchId = mtb.ufnMTBBatchGetByStatBatchLogId( @siStatBatchLogId ) AND MTBDataSetName = N'MTBDailyCustNegBalDays30and90Delta'
		;

		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert MTBDailyCustNegBalDays30and90Delta into mtb.CustomerAccountBulk'
		...
		FROM [mtb].[ufnMTBDailyCustNegBalDays30and90Delta_TranslateRowToCell]( @siStatBatchLogId ) t
