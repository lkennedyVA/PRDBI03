...
GO
/****************************************************************************************
	Name: uspPNCPayerTrxnShortTermStatsMixed_TransferToAtomicStat
	Generator executed by: VALIDRS\LWhiting
	Generated code version: B40B5A88-D563-45CF-8861-B10945632017
	Generation set version: A13233E5-51BF-4F78-A24A-4989C9D7764C
	Description: Upsert pnc.PayerBulk from the relevant unpivoted stats of PNCPayerTrxnShortTermStatsFullSet.
		
	History:
		2019-07-31 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCPayerTrxnShortTermStatsMixed_TransferToAtomicStat]
	(
...
BEGIN
	...
	;
	SELECT @iPNCBatchDataSetRefreshLogId = pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( @siStatBatchLogId, N'PNCPayerTrxnShortTermStatsDelta' )
	;
	
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iPNCBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iPNCBatchDataSetRefreshLogId ), N'NULL' ) + N' via pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( ' + CONVERT( nvarchar(50), @siStatBatchLogId ) + N', N''PNCPayerTrxnShortTermStatsDelta'' )'
	END

	EXEC pnc.uspPNCBatchDataSetRefreshLogTransferInitiate
		 @piStatBatchLogId = @siStatBatchLogId
		,@pnvPNCDataSetName = N'PNCPayerTrxnShortTermStatsDelta'
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		--AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCPayerTrxnShortTermStatsFullSet x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		WHERE x.[ObjectName] = N'PNCPayerTrxnShortTermStatsDelta'
			AND x.IsTransported = 1
		;
		SELECT @biDataSetRowCount = NumberOfRecordsInDataSetRefresh 
		FROM DHayes.dbo.PNCBatchDataSetRefreshLog 
		WHERE PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) AND PNCDataSetName = N'PNCPayerTrxnShortTermStatsDelta'
		;

		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCPayerTrxnShortTermStatsFullSet into pnc.PayerBulk'
		...
		FROM [pnc].[ufnPNCPayerTrxnShortTermStatsFullSet_TranslateRowToCell]( @siStatBatchLogId ) t
