...
GO
/****************************************************************************************
	Name: uspPNCKCPTrxnStatsMixed_TransferToAtomicStat
	Generator executed by: VALIDRS\LWhiting
	Generated code version: B5F40CC6-81B2-445E-B637-2F2A1D7957E6
	Generation set version: A13233E5-51BF-4F78-A24A-4989C9D7764C
	Description: Upsert pnc.KCPBulk from the relevant unpivoted stats of PNCKCPTrxnStatsFullSet.
		
	History:
		2019-07-31 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCKCPTrxnStatsMixed_TransferToAtomicStat]
	(
...
BEGIN
	...
	;
	SELECT @iPNCBatchDataSetRefreshLogId = pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( @siStatBatchLogId, N'PNCKCPTrxnStatsDelta' )
	;
	
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iPNCBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iPNCBatchDataSetRefreshLogId ), N'NULL' ) + N' via pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( ' + CONVERT( nvarchar(50), @siStatBatchLogId ) + N', N''PNCKCPTrxnStatsDelta'' )'
	END

	EXEC pnc.uspPNCBatchDataSetRefreshLogTransferInitiate
		 @piStatBatchLogId = @siStatBatchLogId
		,@pnvPNCDataSetName = N'PNCKCPTrxnStatsDelta'
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		--AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCKCPTrxnStatsFullSet x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		WHERE x.[ObjectName] = N'PNCKCPTrxnStatsDelta'
			AND x.IsTransported = 1
		;
		SELECT @biDataSetRowCount = NumberOfRecordsInDataSetRefresh 
		FROM DHayes.dbo.PNCBatchDataSetRefreshLog 
		WHERE PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) AND PNCDataSetName = N'PNCKCPTrxnStatsDelta'
		;

		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCKCPTrxnStatsFullSet into pnc.KCPBulk'
		...
		FROM [pnc].[ufnPNCKCPTrxnStatsFullSet_TranslateRowToCell]( @siStatBatchLogId ) t
