...
GO
/****************************************************************************************
	Name: uspPNCKCPTrxnShortTermStatsDelta_TransferToAtomicStat
	Generator executed by: VALIDRS\LWhiting
	Generated code version: CD2DC3F8-377E-4468-827D-F0E34BF979EB
	Generation set version: 23166884-8D46-46C0-8D72-6A1F3A75BEE7
	Description: Upsert pnc.KCPBulk from the relevant unpivoted stats of PNCKCPTrxnShortTermStatsDelta.
		
	History:
		2019-08-03 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCKCPTrxnShortTermStatsDelta_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCKCPTrxnShortTermStatsDelta x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCKCPTrxnShortTermStatsDelta into pnc.KCPBulk'
		...
		FROM [pnc].[ufnPNCKCPTrxnShortTermStatsDelta_TranslateRowToCell]( @siStatBatchLogId ) t
