...
GO
/****************************************************************************************
	Name: uspPNCPayerTrxnActivity180StatsDelta_TransferToAtomicStat
	Generator Executed By: VALIDRS\LWhiting
	Generated Version: FBF3113D-764E-43FA-A2D0-DEF185998E62
	Description: Generated user-defined function.
		
	History:
		2019-07-29 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCPayerTrxnActivity180StatsDelta_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCPayerTrxnActivity180StatsDelta x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCPayerTrxnActivity180StatsDelta into pnc.PayerBulk'
		...
		FROM [pnc].[ufnPNCPayerTrxnActivity180StatsDelta_TranslateRowToCell]( @siStatBatchLogId ) t
