...
*/
ALTER PROCEDURE [ftb].[uspBatchStatBatchLogXrefEstablish]
	( 
...
BEGIN
	...
	SELECT
		@iBatchId = BatchId
	FROM [ftb].[BatchStatBatchLogXref]
	...
	BEGIN -- Look in [???Risk].[dbo].[StatBatch] for the latest BatchId to work with.
		...
		FROM [FTBRisk].[dbo].[StatBatch] AS b
		WHERE b.StatBatchTransferInitiatedDatetime IS NULL 
			AND b.StatBatchDeltaPromotionComplete = 0 
			AND NOT EXISTS( SELECT 'X' FROM [ftb].[BatchStatBatchLogXref] AS x WHERE x.BatchId = b.StatBatchId )  
		;
	...
	IF @iStatBatchLogId IS NOT NULL 
		...
		AND ( ISNULL( ( SELECT bl.BatchStartDate FROM [AtomicStat].[stat].[BatchLog] AS bl WHERE bl.BatchLogId = @iStatBatchLogId AND bl.OrgId = @iClientOrgId ), CONVERT( datetime2(0), '1901-01-01' ) ) > ISNULL( ( SELECT b.StatBatchInitiatedDatetime FROM [FTBRisk].[dbo].[StatBatch] AS b WHERE b.StatBatchId = @iBatchId ), CONVERT( datetime2(0), '9999-12-31' ) ) )
	...
	BEGIN
		...
		INSERT INTO [ftb].[BatchStatBatchLogXref] 
			( 
				 BatchId
				,StatBatchLogId 
			)
		...
		WHERE NOT EXISTS( SELECT 'X' FROM [ftb].[BatchStatBatchLogXref] AS x WHERE x.BatchId = @iBatchId ) -- if @iBatchId already exists in any row, we can't add it again.
			AND NOT EXISTS( SELECT 'X' FROM [ftb].[BatchStatBatchLogXref] AS x WHERE x.StatBatchLogId = @iStatBatchLogId ) -- if @iStatBatchLogId already exists in any row, we can't add it again, either.
		*/
		WHERE NOT EXISTS( SELECT 'X' FROM [ftb].[BatchStatBatchLogXref] AS x WHERE x.BatchId = @iBatchId AND x.StatBatchLogId = @iStatBatchLogId ) -- if @iBatchId already exists in any row, we can't add it again.
		;
