...
GO
/****************************************************************************************
	Name: uspPNCPayerTrxnShortTermStatsDelta_TransferToAtomicStat
	Generator executed by: VALIDRS\LWhiting
	Generated code version: BDF285CD-C074-463A-BC1A-528A830879BD
	Generation set version: 23166884-8D46-46C0-8D72-6A1F3A75BEE7
	Description: Upsert pnc.PayerBulk from the relevant unpivoted stats of PNCPayerTrxnShortTermStatsDelta.
		
	History:
		2019-08-03 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCPayerTrxnShortTermStatsDelta_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCPayerTrxnShortTermStatsDelta x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCPayerTrxnShortTermStatsDelta into pnc.PayerBulk'
		...
		FROM [pnc].[ufnPNCPayerTrxnShortTermStatsDelta_TranslateRowToCell]( @siStatBatchLogId ) t
