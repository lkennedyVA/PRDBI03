...
ALTER   PROCEDURE [precalc].[uspFinancialKCPBatchRunValuePrepare_recentDHayesMods]
	(
		 @psiBatchLogId INT = NULL OUTPUT -- if NULL, then grab the last BatchLogId from precalc.BatchRunValue
...
DECLARE 
	...
	--,@siBatchLogId INT = ISNULL( @psiBatchLogId, ( ISNULL( ( /*cheating for the time being*/ SELECT MAX( x.BatchLogId ) FROM precalc.Parameter AS x WHERE x.BatchLogId > 0 ), 0 ) + 1 ) )
--DECLARE 
...
DECLARE @tbParameter table ( RowId int IDENTITY(1,1) NOT NULL, BatchLogId INT NOT NULL, ParameterMessage varchar(128) NOT NULL, ParameterValue nvarchar(128) NULL )
;
