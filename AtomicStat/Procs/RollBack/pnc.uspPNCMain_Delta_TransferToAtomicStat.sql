...
GO
ALTER PROCEDURE [pnc].[uspPNCMain_Delta_TransferToAtomicStat]
	(
...
BEGIN
	...
	BEGIN
		...
		PRINT NCHAR(009) + REPLICATE( N'=', 80 )
		IF EXISTS( SELECT 'X' FROM DHayes.dbo.PNCBatchDataSetRefreshLog WHERE PNCBatchId = @siPNCBatchId AND PNCDataSetName = N'PNCPayerTrxnShortTermStatsDelta' AND PNCDataSetTransferCompletedDatetime IS NULL )
			EXEC [pnc].[uspPNCPayerTrxnShortTermStatsDelta_TransferToAtomicStat] @psiStatBatchLogId = @siStatBatchLogId, @ptiDebug = @ptiDebug
		;
