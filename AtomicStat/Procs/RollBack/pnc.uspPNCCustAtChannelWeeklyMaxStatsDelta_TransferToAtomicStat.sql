...
GO
/****************************************************************************************
	Name: uspPNCCustAtChannelWeeklyMaxStatsDelta_TransferToAtomicStat
	Generator Executed By: VALIDRS\LWhiting
	Generated Version: BFD71E8D-7BF7-4F18-8A1D-85FC3BC001CA
	Description: Generated user-defined function.
		
	History:
		2019-07-29 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCCustAtChannelWeeklyMaxStatsDelta_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCCustAtChannelWeeklyMaxStatsDelta x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCCustAtChannelWeeklyMaxStatsDelta into pnc.CustomerChannelBulk'
		...
		FROM [pnc].[ufnPNCCustAtChannelWeeklyMaxStatsDelta_TranslateRowToCell]( @siStatBatchLogId ) t
