...
/*
	...
	Description: this stored procedure establishes a cross-ref 
		between a AtomicStat.stat.BatchLog.BatchLogId and a TDBRisk.dbo.StatBatch.StatBatchId,
		unless the BatchLogId already exists in AtomicStat.tdb.BatchStatBatchLogXref.StatBatchLogId.

		In either case, the corresponding TDBRisk.dbo.StatBatch.StatBatchId value is returned 
	...
	Example:
		...
		;
		EXEC tdb.uspBatchStatBatchLogXrefEstablish @piStatBatchLogId = @iStatBatchLogId, @piBatchId = @iBatchId OUTPUT 
		;
		SELECT StatBatchLogId = @iStatBatchLogId, BatchId = @iBatchId
		;
		SELECT * FROM [tdb].[BatchStatBatchLogXref] WHERE StatBatchLogId = @iStatBatchLogId
		;
...
*/
ALTER PROCEDURE [tdb].[uspBatchStatBatchLogXrefEstablish]
	( 
...
BEGIN
	...
	BEGIN -- Look in TDBRisk.dbo.StatBatch for the latest BatchId to work with.
		...
		WHERE b.StatBatchTransferInitiatedDatetime IS NULL 
			...
			AND NOT EXISTS( SELECT 'X' FROM [tdb].[BatchStatBatchLogXref] AS x WHERE x.BatchId = b.StatBatchId )  
		;
	...
	END -- Look in TDBRisk.dbo.StatBatch for the latest BatchId to work with.


	IF @iStatBatchLogId IS NOT NULL 
		AND @iBatchId IS NOT NULL
		AND @iStatBatchLogId <> @iBatchId
		-- AtomicStat.stat.BatchLog.BatchStartDate must have a timestamp more recent than TDBRisk.dbo.Batch.BatchInitiatedDatetime...
		AND ( ISNULL( ( SELECT bl.BatchStartDate FROM AtomicStat.stat.BatchLog bl WHERE bl.BatchLogId = @iStatBatchLogId AND bl.OrgId = 100010 ), CONVERT( datetime2(0), '1901-01-01' ) ) > ISNULL( ( SELECT b.StatBatchInitiatedDatetime FROM TDBRisk.dbo.StatBatch AS b WHERE b.StatBatchId = @iBatchId ), CONVERT( datetime2(0), '9999-12-31' ) ) )
		-- AtomicStat.stat.BatchLog.OrgId must be 100010 (TDB)...
		AND ISNULL( ( SELECT bl.OrgId FROM AtomicStat.stat.BatchLog bl WHERE bl.BatchLogId = @iStatBatchLogId ), CONVERT( int, 0 ) ) = 100010
	BEGIN
		...
		INSERT INTO [tdb].[BatchStatBatchLogXref] 
			( 
				 BatchId
				,StatBatchLogId 
			)
		...
		WHERE NOT EXISTS( SELECT 'X' FROM [tdb].[BatchStatBatchLogXref] AS x WHERE x.BatchId = @iBatchId ) -- if @iBatchId already exists in any row, we can't add it again.
			AND NOT EXISTS( SELECT 'X' FROM [tdb].[BatchStatBatchLogXref] AS x WHERE x.StatBatchLogId = @iStatBatchLogId ) -- if @iStatBatchLogId already exists in any row, we can't add it again, either.
		;
