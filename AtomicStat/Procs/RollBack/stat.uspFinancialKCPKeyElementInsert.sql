...
ALTER PROCEDURE [stat].[uspFinancialKCPKeyElementInsert]
	(
		...
		,@psiBatchLogId smallint = NULL OUTPUT
	)
AS
BEGIN	
	SET NOCOUNT ON;
	...
	DECLARE @dtTimerDate datetime2(7) = SYSDATETIME()
		...
		,@biRowCount bigint
		,@siBatchLogId smallint = ISNULL( @psiBatchLogId, ( SELECT MAX( BatchLogId ) FROM [stat].[BatchLog] ) ) -- assumption is that if we are running without an explicit @psiBatchLogId value, then we want the last BatchLogId added in stat.BatchLog.
		;
