...
GO
/****************************************************************************************
	Name: uspPNCKCPTrxnShortTermStatsFullSet_TransferToAtomicStat
	Generator Executed By: VALIDRS\LWhiting
	Generated Version: 503EB048-A91D-4764-A473-0E0738BC7320
	Description: Generated user-defined function.
		
	History:
		2019-07-29 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCKCPTrxnShortTermStatsFullSet_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iPNCBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iPNCBatchDataSetRefreshLogId ), N'NULL' ) + N' via pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( ' + CONVERT( nvarchar(50), @siStatBatchLogId ) + N', N''PNCKCPTrxnShortTermStatsFullSet'' )'
	END

	EXEC pnc.uspPNCBatchDataSetRefreshLogTransferInitiate
		 @piStatBatchLogId = @siStatBatchLogId
		,@pnvPNCDataSetName = N'PNCKCPTrxnShortTermStatsFullSet'
	...
	BEGIN -- are there rows to process?
		...
		WHERE x.[ObjectName] = N'PNCKCPTrxnShortTermStatsFullSet'
			AND x.IsTransported = 1
		;
		SELECT @biDataSetRowCount = NumberOfRecordsInDataSetRefresh 
		FROM DHayes.dbo.PNCBatchDataSetRefreshLog 
		WHERE PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) AND PNCDataSetName = N'PNCKCPTrxnShortTermStatsFullSet'
		;
