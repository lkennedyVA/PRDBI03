...
*****************************************************************************************/
ALTER   PROCEDURE [kegen].[uspFinancialKCPClearedCheckNumber](
	...
	BEGIN	
		...
		WHERE s.StatValue IS NOT NULL
			...
			AND NOT EXISTS (SELECT 'X'
					FROM [stat].[StatTypeNchar50] st
					WHERE s.KeyElementId = st.KeyElementId); -- KeyElementId is suposed to be unique, and therefore should suffice for performing this check. LSW
