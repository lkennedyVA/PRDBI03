...
GO
/****************************************************************************************
	Name: [report].[uspFinancialKCPKeyElement]
	...
	Description: 
		...
		Finally, we insert the Financial KCP KeyElement set into [report].[KeyElement].
...
*****************************************************************************************/
ALTER PROCEDURE [report].[uspFinancialKCPKeyElement](
	...
	BEGIN
		...
		BEGIN
				...
				SET s.HashId = CONVERT(binary(64)
							,HASHBYTES( N'SHA2_512'
								...
								,CONVERT(nvarchar(512)
									,REPLACE( REPLACE( REPLACE( @nvKeyTemplatePreprocessed, '[{CustomerNumber}]', CONVERT(nvarchar(50),c.CustomerNumber ) ) 
			...
			SELECT TRY_CONVERT(nvarchar(50),c.CustomerNumber)
				,TRY_CONVERT(nvarchar(50),p.RoutingNumber)	
