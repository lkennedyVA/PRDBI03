...
GO
/****************************************************************************************
	Name: [report].[uspFinancialCustomerKeyElement]
	Created By: Chris Sharp 
	Description: 
		Assign the KeyTemplatePreprocessed string for the KeyTypeId using the 
	...
	History:
		2018-11-07 - CBS - Created	
*****************************************************************************************/
ALTER PROCEDURE [report].[uspFinancialCustomerKeyElement](
	 @psiBatchLogId SMALLINT --= 36
...
BEGIN
	...
	DECLARE @iPageSize int = @piPageSize
		...
		,@siKeyTypeId smallint = [stat].[ufnKeyTypeIdByName]('Financial Customer')
	...
	SELECT @iPartitionCount = (COUNT(1) / @iPageSize) + 1
		...
		,@iPartitionNumber = 0
	FROM  [kegen].[KCP_PNC_SurrogateKeyXref]  --Whats the precalc table for Customer
	...
	BEGIN
		...
		SELECT DISTINCT kcp.CustomerId
			,kcp.KeyElementId
			,kcp.PartitionId
		...
		BEGIN
				...
				SET s.HashId = CONVERT(binary(64)
							,HASHBYTES( N'SHA2_512'
								...
								,CONVERT(nvarchar(512)
									,REPLACE( @nvKeyTemplatePreprocessed, '[{CustomerNumber}]', CONVERT(nvarchar(50),c.CustomerNumber ) ) 					
									))),1)		
				FROM #tblSubject s 
			...
			SELECT TRY_CONVERT(nvarchar(50),c.CustomerNumber)
				,s.KeyElementId
				,s.PartitionId
				,s.HashId 
			FROM #tblSubject s 
			...
			SELECT DISTINCT @siIdTypeKeyTypeId
				,TRY_CONVERT(nvarchar(50), @siIdTypeKeyTypeId) 
			...
			SELECT DISTINCT @siCustomerNumberKeyTypeId AS KeyTypeId
				...
				,CONVERT(nvarchar(50), CustomerNumber) AS KeyReferenceValue
				,ISNULL(kr.KeyReferenceId, 0) AS KeyReferenceId
				,s.KeyElementId
				,s.PartitionId
				,s.HashId
