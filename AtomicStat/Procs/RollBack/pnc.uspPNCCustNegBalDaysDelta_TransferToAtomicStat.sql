...
GO
/****************************************************************************************
	Name: uspPNCCustNegBalDaysDelta_TransferToAtomicStat
	Generator Executed By: VALIDRS\LWhiting
	Generated Version: EE616538-0C4D-4135-A7E4-ED4CCD52AE7F
	Description: Generated user-defined function.
		
	History:
		2019-07-29 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCCustNegBalDaysDelta_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCCustNegBalDaysDelta x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCCustNegBalDaysDelta into pnc.CustomerBulk'
		...
		FROM [pnc].[ufnPNCCustNegBalDaysDelta_TranslateRowToCell]( @siStatBatchLogId ) t
