...
GO
/****************************************************************************************
	Name: uspPNCCustAtChannelMonthlyMaxStatsFullSet_TransferToAtomicStat
	Generator Executed By: VALIDRS\LWhiting
	Generated Version: 28E18801-8E26-41F1-8440-C27D20D93FAE
	Description: Generated user-defined function.
		
	History:
		2019-07-29 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCCustAtChannelMonthlyMaxStatsFullSet_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iPNCBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iPNCBatchDataSetRefreshLogId ), N'NULL' ) + N' via pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( ' + CONVERT( nvarchar(50), @siStatBatchLogId ) + N', N''PNCCustAtChannelMonthlyMaxStatsFullSet'' )'
	END

	EXEC pnc.uspPNCBatchDataSetRefreshLogTransferInitiate
		 @piStatBatchLogId = @siStatBatchLogId
		,@pnvPNCDataSetName = N'PNCCustAtChannelMonthlyMaxStatsFullSet'
	...
	BEGIN -- are there rows to process?
		...
		WHERE x.[ObjectName] = N'PNCCustAtChannelMonthlyMaxStatsFullSet'
			AND x.IsTransported = 1
		;
		SELECT @biDataSetRowCount = NumberOfRecordsInDataSetRefresh 
		FROM DHayes.dbo.PNCBatchDataSetRefreshLog 
		WHERE PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) AND PNCDataSetName = N'PNCCustAtChannelMonthlyMaxStatsFullSet'
		;
