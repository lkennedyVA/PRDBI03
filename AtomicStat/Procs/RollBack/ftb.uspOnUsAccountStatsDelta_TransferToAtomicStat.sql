...
*****************************************************************************************/
ALTER PROCEDURE [ftb].[uspOnUsAccountStatsDelta_TransferToAtomicStat]
	(
...
BEGIN
	...
	BEGIN -- are there rows to process?
		...
		FROM [FTBRisk].[dbo].[CustAccountStatsDelta] d
		WHERE d.StatBatchId = [ftb].[ufnBatchGetByStatBatchLogId]( @iStatBatchLogId )
		;
		SELECT @biTargetRowCount = COUNT_BIG(1)
		FROM [ftb].[OnUsAccountStatsBulk] d
		;


		TRUNCATE TABLE [ftbprior].[OnUsAccountStatsBulk]
		;
		INSERT INTO [ftbprior].[OnUsAccountStatsBulk]
			(
				 StatBatchLogId
				,AccountNumber
				,ClientOrgId
		...
		SELECT
				...
				,OnUsPayerAccountAvgBalanceCycle1
				,OnUsPayerAccountCurrentBalance
		FROM [ftb].[OnUsAccountStatsBulk]
		;

		IF @tiDebug > 0 
		BEGIN
			PRINT CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + REPLICATE( NCHAR(009), @tiDebug - 1 ) + N'Upsert [ftb].[' + @nvDataFocus + N'Bulk]'
...
/*
		...
		FROM [FTBRisk].[dbo].[Account] AS a
			INNER JOIN [ftb].[OnUsAccountStatsBulk] u
				ON a.AccountNumber = u.AccountNumber
					AND a.ClientOrgId = u.ClientOrgId
			INNER JOIN [FTBRisk].[dbo].[AccountStatus] AS astat 
				ON a.[AccountStatusId] = astat.[AccountStatusId]
			LEFT JOIN [FTBRisk].[dbo].[CustAccountStatsDelta] AS casd
				ON a.[AccountNumber] = casd.[CustomerAccountNumber]
					AND casd.[StatBatchId] = [ftb].[ufnBatchGetByStatBatchLogId]( @iStatBatchLogId )
			LEFT JOIN [FTBRisk].[dbo].[AccountHoldCodeRestrictionXref] AS aholdx 
				ON a.[AccountHoldCode] = aholdx.[AccountHoldCode]
			LEFT JOIN [FTBRisk].[dbo].[AccountTypeDetail] AS atd
...
*/
		...
		FROM [FTBRisk].[dbo].[vwOnUsPayerAccountStatsDelta] AS t
			INNER JOIN [ftb].[OnUsAccountStatsBulk] u
				ON t.PayerAccountNumber = u.AccountNumber
					AND t.PayerClientOrgId = u.ClientOrgId
			INNER JOIN [FTBRisk].[dbo].[Account] a
		...
		IF @tiDebug > 0 
		BEGIN
			SET @nvMessage = CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + REPLICATE( NCHAR(009), @tiDebug - 1 ) + CONVERT( nvarchar(50), @biRowCount ) + N' rows updated in [ftb].[' + @nvDataFocus + N'Bulk].'
		...
		SET @biRowCount = 0
		;
		INSERT INTO [ftb].[OnUsAccountStatsBulk]
			(
				 StatBatchLogId
				,AccountNumber
				,ClientOrgId
...
/*		
		...
		FROM [FTBRisk].[dbo].[Account] AS a
			INNER JOIN [FTBRisk].[dbo].[AccountStatus] AS astat 
				ON a.[AccountStatusId] = astat.[AccountStatusId]
			LEFT JOIN [FTBRisk].[dbo].[CustAccountStatsDelta] AS casd -- TO-DO: ask Diana about how we should obtain the CycleDate
				ON a.[AccountNumber] = casd.[CustomerAccountNumber]
					AND casd.[StatBatchId] = [ftb].[ufnBatchGetByStatBatchLogId]( @iStatBatchLogId )
			LEFT JOIN [FTBRisk].[dbo].[AccountHoldCodeRestrictionXref] AS aholdx 
				ON a.[AccountHoldCode] = aholdx.[AccountHoldCode]
		...
		--where @biTargetRowCount = 0 or not exists
				(
					SELECT 'X'
					FROM [ftb].[OnUsAccountStatsBulk] AS x
...
*/
		...
		IF @tiDebug > 0 
		BEGIN
			SET @nvMessage = CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + REPLICATE( NCHAR(009), @tiDebug - 1 ) + CONVERT( nvarchar(50), @biRowCount ) + N' rows inserted into [ftb].[' + @nvDataFocus + N'Bulk].'
			RAISERROR ( @nvMessage, 0, 1 ) WITH NOWAIT;
		END
