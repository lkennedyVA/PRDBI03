...
GO
/****************************************************************************************
	Name: uspPNCCustAccountSummaryStatsDelta_TransferToAtomicStat
	Generator Executed By: VALIDRS\LWhiting
	Generated Version: 217CBF6F-C538-4127-85A1-7C4FED209A9B
	Description: Generated user-defined function.
		
	History:
		2019-07-29 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCCustAccountSummaryStatsDelta_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCCustAccountSummaryStatsDelta x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCCustAccountSummaryStatsDelta into pnc.CustomerBulk'
		...
		FROM [pnc].[ufnPNCCustAccountSummaryStatsDelta_TranslateRowToCell]( @siStatBatchLogId ) t
