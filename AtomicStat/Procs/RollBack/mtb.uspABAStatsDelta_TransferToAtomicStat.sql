...
GO
/****************************************************************************************
	Name: mtb.uspABAStatsDelta_TransferToAtomicStat
	...
	History:
		2020-07-02 - VALIDRS\LWhiting - Created based upon [t d b].[uspABAStatsDelta_TransferToAtomicStat]

*****************************************************************************************/
ALTER PROCEDURE [mtb].[uspABAStatsDelta_TransferToAtomicStat]
	(
...
BEGIN
	...
	;
	SELECT @iBatchDataSetRefreshLogId = [mtb].ufnBatchDataSetRefreshLogIdGetByBatchLogIdDataSetName( @iStatBatchLogId, N'ABAStatsDelta' )
	;
	
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iBatchDataSetRefreshLogId ), N'NULL' ) + N' via [mtb].ufnBatchDataSetRefreshLogIdGetByBatchLogIdDataSetName( ' + CONVERT( nvarchar(50), @iStatBatchLogId ) + N'}, N''ABAStatsDelta'' )'
	END
	...
	IF @iBatchDataSetRefreshLogId IS NOT NULL 
		AND EXISTS( SELECT 'X' FROM [MTBRisk].[dbo].[ABAStatsDelta] x WHERE x.StatBatchId = [mtb].ufnBatchGetByStatBatchLogId( @iStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		--WHERE StatBatchId = [mtb].ufnBatchGetByStatBatchLogId( @iStatBatchLogId ) AND DataSetName = N'ABAStatsDelta'
		--;
		...
		FROM [MTBRisk].[dbo].[ABAStatsDelta] t
