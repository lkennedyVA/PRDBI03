...
GO
/****************************************************************************************
	Name: uspPNCCustAtChannelStatsMixed_TransferToAtomicStat
	Generator executed by: VALIDRS\LWhiting
	Generated code version: BD03AD19-E45D-45BA-B759-B92C43B4C01F
	Generation set version: A13233E5-51BF-4F78-A24A-4989C9D7764C
	Description: Upsert pnc.CustomerChannelBulk from the relevant unpivoted stats of PNCCustAtChannelStatsFullSet.
		
	History:
		2019-07-31 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCCustAtChannelStatsMixed_TransferToAtomicStat]
	(
...
BEGIN
	...
	;
	SELECT @iPNCBatchDataSetRefreshLogId = pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( @siStatBatchLogId, N'PNCCustAtChannelStatsDelta' )
	;
	
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iPNCBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iPNCBatchDataSetRefreshLogId ), N'NULL' ) + N' via pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( ' + CONVERT( nvarchar(50), @siStatBatchLogId ) + N', N''PNCCustAtChannelStatsDelta'' )'
	END

	EXEC pnc.uspPNCBatchDataSetRefreshLogTransferInitiate
		 @piStatBatchLogId = @siStatBatchLogId
		,@pnvPNCDataSetName = N'PNCCustAtChannelStatsDelta'
	...
	IF @iPNCBatchDataSetRefreshLogId IS NOT NULL 
		--AND EXISTS( SELECT 'X' FROM DHayes.dbo.PNCCustAtChannelStatsFullSet x WHERE x.PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) )
	BEGIN -- are there rows to process?
		...
		WHERE x.[ObjectName] = N'PNCCustAtChannelStatsDelta'
			AND x.IsTransported = 1
		;
		SELECT @biDataSetRowCount = NumberOfRecordsInDataSetRefresh 
		FROM DHayes.dbo.PNCBatchDataSetRefreshLog 
		WHERE PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) AND PNCDataSetName = N'PNCCustAtChannelStatsDelta'
		;

		IF @ptiDebug > 0 
		BEGIN
			PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Upsert PNCCustAtChannelStatsFullSet into pnc.CustomerChannelBulk'
		...
		FROM [pnc].[ufnPNCCustAtChannelStatsFullSet_TranslateRowToCell]( @siStatBatchLogId ) t
