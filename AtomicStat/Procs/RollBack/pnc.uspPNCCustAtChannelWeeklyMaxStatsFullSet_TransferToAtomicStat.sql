...
GO
/****************************************************************************************
	Name: uspPNCCustAtChannelWeeklyMaxStatsFullSet_TransferToAtomicStat
	Generator Executed By: VALIDRS\LWhiting
	Generated Version: 421E209D-075F-4F31-979E-F1156EC632E6
	Description: Generated user-defined function.
		
	History:
		2019-07-29 - VALIDRS\LWhiting - Created via generator.
*****************************************************************************************/
ALTER   PROCEDURE [pnc].[uspPNCCustAtChannelWeeklyMaxStatsFullSet_TransferToAtomicStat]
	(
...
BEGIN
	...
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iPNCBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iPNCBatchDataSetRefreshLogId ), N'NULL' ) + N' via pnc.ufnPNCBatchDataSetRefreshLogIdGetByBatchLogIdPNCDataSetName( ' + CONVERT( nvarchar(50), @siStatBatchLogId ) + N', N''PNCCustAtChannelWeeklyMaxStatsFullSet'' )'
	END

	EXEC pnc.uspPNCBatchDataSetRefreshLogTransferInitiate
		 @piStatBatchLogId = @siStatBatchLogId
		,@pnvPNCDataSetName = N'PNCCustAtChannelWeeklyMaxStatsFullSet'
	...
	BEGIN -- are there rows to process?
		...
		WHERE x.[ObjectName] = N'PNCCustAtChannelWeeklyMaxStatsFullSet'
			AND x.IsTransported = 1
		;
		SELECT @biDataSetRowCount = NumberOfRecordsInDataSetRefresh 
		FROM DHayes.dbo.PNCBatchDataSetRefreshLog 
		WHERE PNCBatchId = pnc.ufnPNCBatchGetByStatBatchLogId( @siStatBatchLogId ) AND PNCDataSetName = N'PNCCustAtChannelWeeklyMaxStatsFullSet'
		;
