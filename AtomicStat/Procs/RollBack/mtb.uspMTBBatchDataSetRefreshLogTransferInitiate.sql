USE [AtomicStat]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
/*

	Name: uspMTBBatchDataSetRefreshLogTransferInitiate

	Created By: VALIDRS\LWhiting
	
	Description: .

	Example:

		DECLARE @iStatBatchLogId int = 30113
			,@siMTBBatchId smallint = NULL
			,@iMTBBatchDataSetRefreshLogId int = NULL
			,@iDataSetRemainCount int = NULL
		;
		EXEC mtb.uspMTBBatchDataSetRefreshLogTransferInitiate @piStatBatchLogId = @iStatBatchLogId, @psiMTBBatchId = @siMTBBatchId OUTPUT, @piMTBBatchDataSetRefreshLogId = @iMTBBatchDataSetRefreshLogId OUTPUT, @piDataSetRemainCount = @iDataSetRemainCount OUTPUT
		;
		SELECT StatBatchLogId = @iStatBatchLogId, MTBBatchId = @siMTBBatchId
		;
		SELECT * FROM [DHayes].[dbo].[MTBBatchDataSetRefreshLog] WHERE MTBBatchId = @siMTBBatchId
		;
		SELECT * FROM [DHayes].[dbo].[MTBBatch] WHERE MTBBatchId = @siMTBBatchId
		;
		
	History:
		2019-03-15 - LSW - Created

*/
ALTER PROCEDURE [mtb].[uspMTBBatchDataSetRefreshLogTransferInitiate]
	( 
		 @piStatBatchLogId int -- AtomicStat works by way of Stat.stat.BatchLog.BatchLogId
		,@pnvMTBDataSetName nvarchar(255) = NULL
		,@psiMTBBatchId smallint OUTPUT -- returned as verification; if NULL, then something went wrong.
		,@piMTBBatchDataSetRefreshLogId int OUTPUT -- returns the Id of a selected data set.
		,@piDataSetRemainCount int OUTPUT
		,@ptiDebug tinyint = 0 -- 0 = no feedback, 1+ = depth of output feedback to Message tab.
	) 
AS 
BEGIN

	SET @ptiDebug = CASE WHEN @ptiDebug > 0 THEN @ptiDebug + 1 ELSE 0 END
	;
	DECLARE @nvThisSourceCode NVARCHAR(4000);
	
	IF @ptiDebug > 0 
	BEGIN
		SET @nvThisSourceCode = dbo.ufnExecutingObjectString( @@SPID, @@PROCID, HOST_NAME(), @@SERVERNAME, DB_NAME(), SUSER_SNAME() )
		;
		PRINT N''
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + @nvThisSourceCode
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' Begin...'
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@piStatBatchLogId = ' + CONVERT( nvarchar(50), @piStatBatchLogId )
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@psiMTBBatchId = ' + ISNULL( CONVERT( nvarchar(50), @psiMTBBatchId ), N'NULL' )
		PRINT N''
	END

	DECLARE @iStatBatchLogId int = @piStatBatchLogId
		,@siMTBBatchId smallint = @psiMTBBatchId
		,@iMTBBatchDataSetRefreshLogId int
		,@dt2MTBBatchDataSetRefreshLogTransferInitiate datetime2(0)
	;
	EXEC mtb.uspMTBBatchTransferInitiate @piStatBatchLogId = @iStatBatchLogId, @psiMTBBatchId = @siMTBBatchId OUTPUT
	;
	SET @siMTBBatchId = mtb.ufnMTBBatchGetByStatBatchLogId( @iStatBatchLogId )
	;
	SELECT 
		@iMTBBatchDataSetRefreshLogId = MIN( MTBBatchDataSetRefreshLogId ) 
	FROM mtb.ufnMTBBatchDataSetRefreshLogGetByBatchLogId( @iStatBatchLogId ) 
	WHERE MTBDataSetReadyForTransferYN = N'Y' 
		AND MTBDataSetTransferredYN = N'N' 
		AND MTBDataSetTransferInitiatedDatetime IS NULL
		AND ( @pnvMTBDataSetName IS NULL OR ( @pnvMTBDataSetName = MTBDataSetName ) ) 
	;
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iStatBatchLogId = ' + CONVERT( nvarchar(50), @iStatBatchLogId )
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@siMTBBatchId = ' + ISNULL( CONVERT( nvarchar(50), @siMTBBatchId ), N'NULL' )
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@iMTBBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @iMTBBatchDataSetRefreshLogId ), N'NULL' )
	END

	SET @dt2MTBBatchDataSetRefreshLogTransferInitiate = SYSDATETIME()
	;
	UPDATE [DHayes].[dbo].[MTBBatchDataSetRefreshLog] 
		SET [MTBDataSetTransferInitiatedDatetime] = @dt2MTBBatchDataSetRefreshLogTransferInitiate
		WHERE MTBBatchDataSetRefreshLogId = @iMTBBatchDataSetRefreshLogId 
			AND [MTBDataSetReadyForTransferYN] = N'Y' -- can't initiate the batch data set for AtomicStat transfer if it is still being prepared by the DHayes MTB process.
			AND [MTBDataSetTransferredYN] = N'N'
			AND [MTBDataSetTransferInitiatedDatetime] IS NULL -- can't initiate the batch data set for AtomicStat transfer if it is already marked as initiated.
	;
	
	SELECT 
		@piMTBBatchDataSetRefreshLogId = mb.MTBBatchDataSetRefreshLogId
	FROM [DHayes].[dbo].[MTBBatchDataSetRefreshLog] AS mb
	WHERE mb.MTBBatchId = @siMTBBatchId 
		AND mb.[MTBDataSetTransferInitiatedDatetime] = @dt2MTBBatchDataSetRefreshLogTransferInitiate
		AND mb.[MTBDataSetTransferCompletedDatetime] IS NULL -- this better be NULL at this point, or we have a problem.
	;
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@piMTBBatchDataSetRefreshLogId = ' + ISNULL( CONVERT( nvarchar(50), @piMTBBatchDataSetRefreshLogId ), N'NULL' )
	END

	SELECT 
		@piDataSetRemainCount = COUNT(1) 
	FROM mtb.ufnMTBBatchDataSetRefreshLogGetByBatchLogId( @iStatBatchLogId ) 
	WHERE MTBDataSetReadyForTransferYN = N'Y' 
		AND MTBDataSetTransferredYN = N'N'
	;
	IF @ptiDebug > 0 
	BEGIN
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + N'@piDataSetRemainCount = ' + ISNULL( CONVERT( nvarchar(50), @piDataSetRemainCount ), N'NULL' )
	END

	IF @ptiDebug > 0 
	BEGIN
		PRINT N''
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + @nvThisSourceCode
		PRINT REPLICATE( NCHAR(009), @ptiDebug - 1 ) + CONVERT( nvarchar(50), SYSDATETIME(), 121 ) + N' ...end.'
		RAISERROR ( N'', 0, 1 ) WITH NOWAIT;
	END

END

GO
