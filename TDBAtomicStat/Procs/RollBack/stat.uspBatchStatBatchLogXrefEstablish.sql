...
BEGIN
	...
	DECLARE @iStatBatchLogId int = @piStatBatchLogId
		...
		,@iClientOrgId int = ( SELECT sg.OrgId FROM stat.StatGroup AS sg WHERE sg.[Name] = ( SELECT TOP 1 [ClientAbbr] FROM [dbo].[Client] ) AND sg.[StatGroupId] = sg.[AncestorStatGroupId] )
	;
